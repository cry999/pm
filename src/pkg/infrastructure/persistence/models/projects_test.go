...
func testProjectToManyPlannedProjectTasks(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, plannedProjectTaskDBTypes, false, plannedProjectTaskColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, plannedProjectTaskDBTypes, false, plannedProjectTaskColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.PlannedProjectTasks().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err = a.L.LoadPlannedProjectTasks(ctx, tx, false, (*[]*Project)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.PlannedProjectTasks); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.PlannedProjectTasks = nil
	if err = a.L.LoadPlannedProjectTasks(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.PlannedProjectTasks); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testProjectToManyAddOpPlannedProjectTasks(t *testing.T) {
	var err error
	...
	var a Project
	var b, c, d, e PlannedProjectTask
	...
	foreigners := []*PlannedProjectTask{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, plannedProjectTaskDBTypes, false, strmangle.SetComplement(plannedProjectTaskPrimaryKeyColumns, plannedProjectTaskColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*PlannedProjectTask{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddPlannedProjectTasks(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.PlannedProjectTasks[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.PlannedProjectTasks[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.PlannedProjectTasks().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
var (
	projectDBTypes = map[string]string{`ID`: `varchar`, `OwnerID`: `varchar`, `Name`: `varchar`, `ElevatorPitch`: `varchar`, `CreatedAt`: `datetime`, `UpdatedAt`: `datetime`}
	_              = bytes.MinRead
)
