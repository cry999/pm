...
type Project struct {
	...
	ElevatorPitch string    `boil:"elevator_pitch" json:"elevator_pitch" toml:"elevator_pitch" yaml:"elevator_pitch"`
...
var ProjectWhere = struct {
	...
	ElevatorPitch whereHelperstring
	...
	ElevatorPitch: whereHelperstring{field: "`projects`.`elevator_pitch`"},
...
var ProjectRels = struct {
	...
	PlannedProjectTasks: "PlannedProjectTasks",
}

// projectR is where relationships are stored.
type projectR struct {
	PlannedProjectTasks PlannedProjectTaskSlice `boil:"PlannedProjectTasks" json:"PlannedProjectTasks" toml:"PlannedProjectTasks" yaml:"PlannedProjectTasks"`
}
...
var (
	projectAllColumns            = []string{"id", "owner_id", "name", "elevator_pitch", "created_at", "updated_at"}
	projectColumnsWithoutDefault = []string{"id", "owner_id", "name", "elevator_pitch"}
...
// PlannedProjectTasks retrieves all the planned_project_task's PlannedProjectTasks with an executor.
func (o *Project) PlannedProjectTasks(mods ...qm.QueryMod) plannedProjectTaskQuery {
	...
	queryMods = append(queryMods,
		qm.Where("`planned_project_tasks`.`project_id`=?", o.ID),
	)

	query := PlannedProjectTasks(queryMods...)
	queries.SetFrom(query.Query, "`planned_project_tasks`")

	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"`planned_project_tasks`.*"})
	}

	return query
}

// LoadPlannedProjectTasks allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (projectL) LoadPlannedProjectTasks(ctx context.Context, e boil.ContextExecutor, singular bool, maybeProject interface{}, mods queries.Applicator) error {
	var slice []*Project
	var object *Project
	...
	query := NewQuery(
		qm.From(`planned_project_tasks`),
		qm.WhereIn(`planned_project_tasks.project_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err != nil {
		return errors.Wrap(err, "failed to eager load planned_project_tasks")
	}

	var resultSlice []*PlannedProjectTask
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice planned_project_tasks")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on planned_project_tasks")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for planned_project_tasks")
	}
	...
	if singular {
		object.R.PlannedProjectTasks = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &plannedProjectTaskR{}
			}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.ProjectID {
				local.R.PlannedProjectTasks = append(local.R.PlannedProjectTasks, foreign)
				if foreign.R == nil {
					foreign.R = &plannedProjectTaskR{}
				}
...
// AddPlannedProjectTasks adds the given related objects to the existing relationships
// of the project, optionally inserting them as new records.
// Appends related to o.R.PlannedProjectTasks.
// Sets related.R.Project appropriately.
func (o *Project) AddPlannedProjectTasks(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*PlannedProjectTask) error {
	var err error
	...
	if o.R == nil {
		o.R = &projectR{
			PlannedProjectTasks: related,
		}
	} else {
		o.R.PlannedProjectTasks = append(o.R.PlannedProjectTasks, related...)
	}

	for _, rel := range related {
		if rel.R == nil {
			rel.R = &plannedProjectTaskR{
				Project: o,
			}
		} else {
			rel.R.Project = o
